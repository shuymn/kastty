...
- 起動 1 コマンドで使える
- ブラウザが自動で開く（任意）
- 余計な操作 UI を出さず、ブラウザ標準 zoom で表示サイズを調整できる
...
kastty --no-open              # ブラウザ自動起動を無効化
kastty --help                 # ヘルプ表示
```

`-` で始まる値を起動対象コマンドへ渡す場合は、`--` で kastty のオプションと区切る（例: `kastty -- htop -d 10`）。
...
- 端末表示（ghostty-web）
- タブタイトル追従（OSC タイトルを反映。`🟡`=connecting、`🔴`=disconnected）
- footer / controls toolbar は提供しない
...
| hello | テキストフレーム (JSON) | 接続確立通知 `{"t":"hello"}` |
...
web/        フロントエンド（ghostty-web）、index.html は Bun HTML imports で自動バンドル、同梱フォントアセット
...
| `cli/` | CLI 引数パーサー | commander（[ADR-0015](../adr/0015-standardize-cli-parsing-with-commander.md)） |
...
readonly は廃止する。クライアント UI、サーバ状態、WS 制御メッセージ、CLI オプションはいずれも提供しない（[ADR-0006](../adr/0006-readonly-double-guard.md)）。
...
| [0006](../adr/0006-readonly-double-guard.md) | readonly を廃止 | Superseded |
...
| [0015](../adr/0015-standardize-cli-parsing-with-commander.md) | CLI 引数解析を commander に統一し未知オプションを fail-fast にする | Accepted |
...
| readonly の実装範囲 | 廃止 | [0006](../adr/0006-readonly-double-guard.md) |
...
6. footer / controls toolbar は表示されない
7. スクロール挙動は ghostty-web 標準に委譲し、auto-scroll トグルを提供しない
8. PTY プロセス終了時にクライアントへ通知される
9. ブラウザ切断後もPTY は維持され、リロードで再接続・現在の画面が表示される
10. `kastty` プロセスはフォアグラウンドでブロックし、`Ctrl+C` または PTY プロセス終了で停止する
11. 同じ `?t=<token>` URL を複数タブで開いた場合、同じ PTY 出力が各タブへ配信される
